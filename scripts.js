...
class CanvasBackground {
    ...
    drawConnections() {
        this.ctx.strokeStyle = 'rgba(0, 255, 34, 0.2)';
        ...
        for (let i = 0; i < this.particles.length; i++) {
            for (let j = i + 1; j < this.particles.length; j++) {
                ...
                if (distance < 200) {
                    ...
                    this.ctx.strokeStyle = `rgba(0, 255, 34, ${opacity * 0.3})`;
                    this.ctx.beginPath();
...
class Particle {
    ...
    draw(ctx) {
        ctx.fillStyle = `rgba(0, 255, 34, ${this.opacity})`;
        ctx.beginPath();
        ctx.arc(this.x, this.y, this.radius, 0, Math.PI * 2);
        ctx.fill();
        
        // Glow effect
        ctx.strokeStyle = `rgba(0, 255, 34, ${this.opacity * 0.5})`;
        ctx.lineWidth = 1;
        ctx.beginPath();
